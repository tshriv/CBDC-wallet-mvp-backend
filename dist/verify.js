...
const runVerification = async () => {
    try {
        ...
        try {
            await axios_1.default.post(`${API_URL}/auth/register`, {
                name: 'User A',
            ...
            if (e.response?.status !== 400)
                throw e; // Ignore if already exists
        ...
        const loginA = await axios_1.default.post(`${API_URL}/auth/login`, {
            phoneNumber: '1234567890',
            password: 'password123',
        });
        ...
        const loginB = await axios_1.default.post(`${API_URL}/auth/login`, {
            phoneNumber: '0987654321',
            password: 'password123',
        });
        ...
        const balanceA1 = await axios_1.default.get(`${API_URL}/wallet/balance`, {
            headers: { Authorization: `Bearer ${tokenA}` },
        });
        console.log(`Balance A: ${balanceA1.data.balance}`);
        // 6. Load Funds A
        console.log('Loading Funds to A (100)...');
        await axios_1.default.post(`${API_URL}/wallet/load`, { amount: 100 }, { headers: { Authorization: `Bearer ${tokenA}` } });
        console.log('Funds Loaded');
        // 7. Check Balance A
        const balanceA2 = await axios_1.default.get(`${API_URL}/wallet/balance`, {
            headers: { Authorization: `Bearer ${tokenA}` },
        });
        console.log(`Balance A: ${balanceA2.data.balance}`);
        // 8. Transfer from A to B
        console.log('Transferring 50 from A to B...');
        await axios_1.default.post(`${API_URL}/wallet/transfer`, { toPhoneNumber: '0987654321', amount: 50 }, { headers: { Authorization: `Bearer ${tokenA}` } });
        console.log('Transfer Successful');
        // 9. Check Balance A
        const balanceA3 = await axios_1.default.get(`${API_URL}/wallet/balance`, {
            headers: { Authorization: `Bearer ${tokenA}` },
        });
        console.log(`Balance A: ${balanceA3.data.balance}`);
        // 10. Get Transactions A
        console.log('Getting Transactions for A...');
        const transactionsA = await axios_1.default.get(`${API_URL}/wallet/transactions`, {
            headers: { Authorization: `Bearer ${tokenA}` },
        });
